...
Login using DoormanAuth0 authentication.
...
sub login : Chained( 'base' ) : PathPart( 'login' ) : Args( 0 ) {
    ...
    # NEED ANSWER: do we need to also check the DoormanAuth0 session here, or only just the Shiny session?
    ...
    # at this point, the Javascript code for Auth0 will take over & hopefully redirect to /users/sign_in/auth0
}
...
sub logout : Chained( 'base' ) : PathPart( 'logout' ) : Args( 0 ) {
    ...
    # clear the user's Shiny session (not Auth0 session)
    print {*STDERR} '<<< DEBUG >>>: in User::logout(), about to call $c->logout to clear Shiny session...', "\n";
    $c->logout;
    
    # DO NOT Set a status message
#    $c->stash->{ status_msg } = 'You have been logged out.';
    
    # DO NOT Send the user to the site's homepage
#    $c->response->redirect( $c->uri_for( '/' ) );

    # redirect to DoormanAuth0 sign_out page,
    # to clear the user's Auth0 session (not Shiny session)
    print {*STDERR} '<<< DEBUG >>>: in User::logout(), about to redirect to DoormanAuth0 sign_out page...', "\n";
    $c->response->redirect( $c->uri_for( '/users/sign_out/auth0' ) );
}
