...
#                print {*STDERR} '<<<DEBUG>>>: found \000 NULL' if ($column_diff_value eq "\000");
            ...
            for (my $row_number = 0; $row_number < $ROWS_MAX; $row_number++) {
                for (my $column_number = 0; $column_number < $COLUMNS_MAX; $column_number++) {
                    $vt_screen_string .= $vt_screen->[$row_number]->[$column_number];
                }
                if ($row_number < ($ROWS_MAX - 1)) {
                    $vt_screen_string .= "\n";
                }
#                else { chomp $vt_screen_string; }
            }
#            print "\033[2J";    #clear the screen
#            print "\033[0;0H"; #jump to 0,0
            `reset`;
#            `clear`;
#            print $vt_screen_clear;
